#!/usr/bin/env bash
# Run the comms-migration (classifier/) test suite with line + branch coverage.
...
if ! "$PY" -c "import pytest" 2>/dev/null; then
  echo "error: pytest not installed. Activate .venv or: $PY -m pip install -r requirements.txt" >&2
  exit 1
fi
...
echo "=== comms-migration coverage (classifier/) ==="
echo "Python: $PY"
echo
