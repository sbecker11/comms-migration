...
- Currently wired to the recruiting funnel only (`shawnbecker.recruiting@gmail.com`). `scbboston@gmail.com` / `personal_hub` was removed from the classifier account list (2026-09-13). `shawn.becker@spexture.com` (Hostinger-hosted) is handled via IMAP on the job-tracker side, not this Gmail classifier.
